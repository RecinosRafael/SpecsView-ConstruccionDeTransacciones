describe('Flujo Oracle: Ejecutar script SQL y configurar columnas ID como IDENTITY', () => {
  ...
  it('Paso 1: Recopilar sentencias ALTER por esquema y del script SQL', () => {
    ...
    const procesarEsquema = (index) => {
      ...
      const sqlPorEsquema = `
        SELECT 'ALTER TABLE '||atc.owner ||'.'||atc.table_name ||' MODIFY ID GENERATED BY DEFAULT ON NULL AS IDENTITY ( START WITH LIMIT VALUE)' AS alter_sql,
               atc.owner
        FROM all_tab_columns atc INNER JOIN all_tables AT ON atc.table_name=at.table_name AND atc.owner=AT.owner
        WHERE atc.column_name = 'ID'
          AND atc.owner = '${esquema.nombre}'
          AND atc.table_name NOT IN ('DATABASECHANGELOG','DATABASECHANGELOGLOCK','WORKFLOW_DEFINITION_DETAIL','HTE_USER_NOTIFICATION')
          AND atc.table_name NOT LIKE '%_AUD'
