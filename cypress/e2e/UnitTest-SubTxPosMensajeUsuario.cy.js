...
describe("Prueba unitaria del Crud Gestor de Transacciones ...", function() {
    ...
    beforeEach(function() {
        ...
        cy.readFile('./JsonData/gestorMensajeUsuarioCondicionadoPOS.json').as('data');
    });
                ...
                .within(() => {
                    ...
                    return cy.wrap(agrupadas[codigoTRX]).each((item) => {
                        ...
                        cy.xpath("//button[.//mat-icon[text()='check']]")
                            .scrollIntoView({ duration: 500, force: true })
                            .click({ force: true });
                        
                        let nombre = "POS Mensaje de Usuario"

                        Generales.procesarRespuestaYReportarConFrame(alias, {
                            numero,
                            describe: `000 -: ${nombre}`,
                            crud: `${nombre}`,
                            descripcion: `Codigo TRX: ${codigoTRX} - Regla de negocio: ${item.reglaNegocio} - Estado: ${item.estado}`
                        })    
